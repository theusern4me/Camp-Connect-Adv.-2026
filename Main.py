...
backgroundThreshold = 40 # Tolerance for color being background
...
firstDerSmplCount = image.shape[1] - firstDerSmplSize + 1 # Number of points derivative is calculated at
...
for y in range(image.shape[1]):
    ...
    for x in range(image.shape[0]):
        dif = int(sum(map(abs, (image[x, y - 1] - bgSample))))
        curScore += backgroundThreshold - dif
        scores.append(curScore)

        if curScore > scores[curMaxIndex]:
            curMaxIndex = x

    edge.append(curMaxIndex)
...
'''kConst = 5
kernel = np.array([[-kConst, -kConst, -kConst],
                    [0, 0, 0],
                    [kConst, kConst, kConst]])
image4 = cv.cvtColor(cv.filter2D(src=image, ddepth=-1, kernel=kernel), cv.COLOR_BGR2GRAY)

image = np.array([[(image[x, y], np.zeros(3))[image4[x, y] > 100] for y in range(image.shape[1])] for x in range(image.shape[0])])

cv.imshow("edge", image.astype(np.uint8))
cv.waitKey(0)'''

smoothEdge = scip.signal.savgol_filter(edge, window_length = windowLength, polyorder = polyOrder) # Smooths edge

for y in range(image.shape[1]): image2[min(rnd(smoothEdge[y]), image.shape[0] - 1), y] = np.array([255, 255, 255]) # Draws edge
for y in range(1, image.shape[1] - 1):
    image[rnd(min(edge[y], image.shape[0] - 1)), y] = np.array([0, 0, 0])
    image[rnd(min(edge[y], image.shape[0] - 1)), y + 1] = np.array([0, 0, 0])
    image[rnd(min(edge[y], image.shape[0] - 1)), y - 1] = np.array([0, 0, 0])
    image[rnd(min(edge[y] + 1, image.shape[0] - 1)), y] = np.array([0, 0, 0])
    image[rnd(min(edge[y] - 1, image.shape[0] - 1)), y] = np.array([0, 0, 0])
...
# Plot cutoff lines
cv.rectangle(image3, ((0, image.shape[0] // 4 + rnd(firstDerMax * 150))), (image.shape[1], image.shape[0] // 4 + rnd(firstDerMax * firstDerThreshold * 150)), (0, 32, 32), -1)
cv.rectangle(image3, ((0, image.shape[0] // 4 - rnd(firstDerMax * 150))), (image.shape[1], image.shape[0] // 4 - rnd(firstDerMax * firstDerThreshold * 150)), (0, 32, 32), -1)
cv.rectangle(image3, (0, image.shape[0] // 2 - rnd(secondDerThreshold * 3000)), (image.shape[1], image.shape[0] // 2 + rnd(secondDerThreshold * 3000)), (0, 0, 64), -1)
cv.rectangle(image3, (0, 3 * image.shape[0] // 4 + rnd(thirdDerThreshold * 20000)), (image.shape[1], 3 * image.shape[0] // 4 - rnd(thirdDerThreshold * 20000)), (32, 0, 32), -1)
...
cv.line(image3, (0, image.shape[0] // 4 - rnd(firstDerMax * firstDerThreshold * 150)), (image.shape[1], image.shape[0] // 4 - rnd(firstDerMax * firstDerThreshold * 150)), (0, 64, 64), 1)
cv.line(image3, (0, image.shape[0] // 2 + rnd(secondDerThreshold * 3000)), (image.shape[1], image.shape[0] // 2 + rnd(secondDerThreshold * 3000)), (0, 0, 128), 1)
cv.line(image3, (0, image.shape[0] // 2 - rnd(secondDerThreshold * 3000)), (image.shape[1], image.shape[0] // 2 - rnd(secondDerThreshold * 3000)), (0, 0, 128), 1)
...
for i in range(firstDerSmplCount - 4):
    cv.circle(image3, (firstDerivative[i + 2][1], int(image.shape[0] // 4 - firstDerivative[i + 2][0] * 150)), 2, (0, 255, 255), thickness = -1) # Draws circles to graph the first derivative
    cv.circle(image3, (secondDerivative[i + 1][1], int(image.shape[0]//2 - secondDerivative[i + 1][0] * 3000)), 2, (0, 0, 255), thickness = -1)
...
print("Number of data points: " + str(len(filteredSlopes2)))

cv.imshow("Detected Edges and Best-Fit Lines", image2.astype(np.uint8)) # Shows images
cv.imshow("Nth Derivitive data", image3.astype(np.uint8))
