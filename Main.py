...
backgroundThreshold = 55 # Tolerance for color being background
...
for y in range(1, image.shape[1] - 1):
    image[rnd(edge[y]), y] = np.array([0, 0, 0])
    image[rnd(edge[y]), y + 1] = np.array([0, 0, 0])
    image[rnd(edge[y]), y - 1] = np.array([0, 0, 0])
    image[rnd(edge[y]) + 1, y] = np.array([0, 0, 0])
    image[rnd(edge[y]) - 1, y] = np.array([0, 0, 0])
